...
const STORAGE_KEY = 'minecraft_generator_current_skript';
...
export default function SkriptPage() {
  ...
  useEffect(() => {
    ...
    setHistory(allHistory.filter(gen => gen.resourceType === 'skript'));
  ...
  const handleCodeUpdate = (updatedCode: string) => {
    if (currentGeneration) {
      ...
      const allHistory = getHistory();
      const otherHistory = allHistory.filter(gen => gen.resourceType !== 'skript');
  ...
  const handleModifyWithAI = async (modificationPrompt: string, currentCode: string) => {
    ...
    try {
      const response = await modifyCode(modificationPrompt, currentCode, 'skript');
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="minecraft-btn minecraft-diamond px-6 py-3 font-bold minecraft-font transition hover:scale-105"
            title="Start a new chat"
          >
            <span className="glow-diamond">✨</span> New Chat
          </button>
        </div>
